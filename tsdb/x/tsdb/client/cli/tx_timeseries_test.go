...
func TestCreateTimeseries(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdCreateTimeseries(), args)
			if tc.err != nil {
...
func TestUpdateTimeseries(t *testing.T) {
	...
	_, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdCreateTimeseries(), args)
	require.NoError(t, err)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			args = append(args, fields...)
			args = append(args, tc.args...)
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdUpdateTimeseries(), args)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
...
func TestDeleteTimeseries(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		tc := tc
		t.Run(tc.desc, func(t *testing.T) {
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdDeleteTimeseries(), append([]string{tc.id}, tc.args...))
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
